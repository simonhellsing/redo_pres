...
export default function SalesPage() {
  ...
  useEffect(() => {
    if (params.id) {
      fetch(`/api/presentations/${params.id}`)
        .then((res) => res.json())
        .then((data) => {
          const { id, companyName, logoUrl, primaryColor, themeMode, customerCompanyName, customerLogoUrl, presentationTitle } = data
          setPresentation({ id, companyName, logoUrl, primaryColor, themeMode, customerCompanyName, customerLogoUrl, presentationTitle })
          setLoading(false)
        })
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-[#faf8f6] flex items-center justify-center">
        <div className="animate-pulse text-[#213327]">Laddar...</div>
      </div>
    )
  }
  ...
  if (!presentation) {
    return (
      <div className="min-h-screen bg-[#faf8f6] flex items-center justify-center">
        <div className="text-[#213327]">Sidan hittades inte</div>
      </div>
    )
  }
    ...
    <div className="min-h-screen bg-[#faf8f6] flex flex-col items-start overflow-clip p-3 sm:p-4 md:p-6 relative w-full pb-20">
      ...
      <div className="flex items-center justify-center px-4 sm:px-12 md:px-32 lg:px-[400px] py-8 sm:py-12 md:py-16 lg:py-20 w-full relative">
        ...
        <div className="absolute inset-0 pointer-events-none overflow-hidden">
          ...
          />
          {/* Vignette - fades grid to background color at edges */}
          <div 
...
// Brand 1 View (Dark Theme - Matching Figma)
function Brand2View({
  presentation,
  onShowExample,
  onInterested,
